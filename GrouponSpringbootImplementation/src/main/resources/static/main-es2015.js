...
let AddCompanyComponent = class AddCompanyComponent {
    ...
    createCompany() {
        if (this.comp.email != null && this.comp.name != null && this.comp.password != null) {
            this.adminService.addCompany(this.comp).subscribe(compFromServer => {
...
let AddCouponComponent = class AddCouponComponent {
    ...
    createCoupon() {
        if (this.coup.amount != null && this.coup.categoryName != null && this.coup.description != null &&
            ...
            this.companyService.addCoupon(this.coup).subscribe(coupFromServer => {
...
let AddCustomerComponent = class AddCustomerComponent {
    ...
    createCustomer() {
        if (this.cust.firstName != null && this.cust.lastName != null && this.cust.email != null &&
            ...
            this.adminService.addCustomer(this.cust).subscribe(custFromServer => {
...
let CompaniesComponent = class CompaniesComponent {
    ...
    deleteCompany(id) {
        this.adminService.deleteCompany(id).subscribe(() => { this.ngOnInit(); }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
                sessionStorage.removeItem("type");
                this.router.navigate(['']);
    ...
    updateCompany() {
        if (this.c.email != null && this.c.name != null && this.c.password != null) {
            this.adminService.editCompany(this.c).subscribe(compFromServer => {
                alert("The Company: " + this.c.name + " was updated");
            }, error => {
                if (error.error == "Session Timeout") {
                    sessionStorage.removeItem("token");
                    sessionStorage.removeItem("type");
    ...
    showAllCompanies() {
        this.adminService.showAllCompanies().subscribe((comp) => {
            this.companies = comp;
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
...
let CompanyCouponsComponent = class CompanyCouponsComponent {
    ...
    showCompanyCoupons() {
        this.companyService.showCompanyCoupons().subscribe((coup) => {
            this.coupons = coup;
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
    ...
    showCompanyCouponsByCategory() {
        this.companyService.showCompanyCouponsbyCategory(this.couponsCategory).subscribe((coup) => {
            this.coupons = coup;
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
    ...
    showCompanyCouponsByMaxPrice() {
        this.companyService.showCouponsbyMaxPrice(this.maxPrice).subscribe((coup) => {
            this.coupons = coup;
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
    ...
    updateCompanyCoupon() {
        if (this.coupon.amount != null && this.coupon.categoryName != null && this.coupon.description != null &&
            ...
            this.companyService.editCoupon(this.coupon).subscribe(coupFromServer => {
...
let CouponsComponent = class CouponsComponent {
    constructor(adminService, customerService, router) {
        this.adminService = adminService;
    ...
    purchaseCoupon(coupon) {
        this.customerService.purchaseCoupon(coupon).subscribe(coupFromServer => {
            alert("The Coup: " + this.coupon.title + " " + this.coupon.description + " was purchased");
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
                sessionStorage.removeItem("type");
    ...
    showAllCoupons() {
        this.adminService.showAllCoupons().subscribe((coup) => {
            this.coupons = coup;
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
    ...
    showCouponsByCategory() {
        console.log(this.couponsCategory);
        this.adminService.showCouponsbyCategory(this.couponsCategory).subscribe((coup) => {
            this.coupons = coup;
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
    ...
    showCouponsByMaxPrice() {
        this.adminService.showCouponsbyMaxPrice(this.maxPrice).subscribe((coup) => {
            this.coupons = coup;
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
...
let CustomersComponent = class CustomersComponent {
    ...
    showAllCustomers() {
        this.adminService.showAllCustomers().subscribe((cust) => {
            this.customers = cust;
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
    ...
    deleteCustomer(id) {
        this.adminService.deleteCustomer(id).subscribe(() => { this.ngOnInit(); }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
                sessionStorage.removeItem("type");
                this.router.navigate(['']);
    ...
    updateCustomer() {
        if (this.c.email != null && this.c.firstName != null && this.c.lastName != null && this.c.password != null) {
            this.adminService.editCustomer(this.c).subscribe(custFromServer => {
                alert("The Customer: " + this.c.firstName + " " + this.c.lastName + " was updated");
            }, error => {
                if (error.error == "Session Timeout") {
                    sessionStorage.removeItem("token");
                    sessionStorage.removeItem("type");
...
let LoggerInformationComponent = class LoggerInformationComponent {
    constructor(loggerInfoService, router) {
    ...
    ngDoCheck() {
        if (sessionStorage.type && this.gotInfo == false) {
            ...
            if (this.loggedInType == "company") {
                this.loggerInfoService.showCompanyInformation().subscribe((comp) => {
                    this.companyLoggedIn = comp;
                    this.gotInfo = true;
                });
            }
            else if (this.loggedInType == "customer") {
                this.loggerInfoService.showCustomerInformation().subscribe((cust) => {
...
LoggerInformationComponent.ctorParameters = () => [
...
let LoginComponent = class LoginComponent {
    ...
    login() {
        this.loginService.login(this.userName, this.password, this.type).subscribe(response => {
            if (response.token) {
        ...
        }, error => { alert("Wrong Password/UserName/Type"); });
    }
    routeToUser(type) {
        switch (type) {
            case "admin":
...
let MyCouponsComponent = class MyCouponsComponent {
    ...
    showCustomerCoupons() {
        this.customerService.showCustomerCoupons().subscribe((coup) => {
            this.customerCoupons = coup;
        }, error => {
            if (error.error == "Session Timeout") {
                sessionStorage.removeItem("token");
                sessionStorage.removeItem("type");
...
class Coupon {
    constructor(id, company, categoryName, title, description, startDate, endDate, amount, price, image) {
        this.id = id;
...
let CompanyService = class CompanyService {
    ...
    showCompanyCouponsbyCategory(category) {
...
let LoggerInformationService = class LoggerInformationService {
    constructor(httpClient) {
        this.httpClient = httpClient;
    }
...
LoggerInformationService.ctorParameters = () => [
    { type: _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpClient"] }
];
